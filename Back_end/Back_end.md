...
- 서버에서는 사용자를 구분하기 위한 기한이 짧은 임시 키 하나를 브라우저에 보내서 쿠키로 저장  
...
- 내가 어떤 웹사이트에 보내는 정보를 다른 누군가 훔쳐보지 못하게 함
- 접속한 사이트가 진품인지 신뢰할 수 있는 사이트인지를 판별

## 대칭키, 비대칭키

## 대칭키
- 매세지를 보내는 쪽과 메시지를 받는 쪽이 메시지를 암호화하고, 이를 다시 메시지를 바꾸는 즉 복호화하는 같은 방식을 공유

 - (예시: @%$$#) 
 이미의 문자열 '키'
 - 4에 3을 곱하면 12
 - 상대방에게 보내고자 하는 메시지를 이'키'와 팜께 어떤 알고리즘에 넣고 돌리면 전혀 알 수 없는 암호문이 만들어진다
 - 12에서 4를 얻어내려먼 3으로 나눠야 하는것처럼 암호문을 받은 사람이 이걸 해독하려면<br>
 키값을 알고, 알고리즘을 거꾸로 돌려야 함<br>
 이 과정울 복호화
 ...
 - 'A'키로 암호화하면 'B'키로만 복호화할 수 있음

 - 'B'키로만 암호화 하면 'A'키로만 풀 수있음
...
![](https://velog.velcdn.com/images%2Fppyooy336%2Fpost%2F80a1ffc6-963f-4e42-a179-99f2fd99a47e%2Fimage.png)
...
#### 동적 웹페이지
![](https://velog.velcdn.com/images%2Fppyooy336%2Fpost%2Fcf90e335-fb26-4758-a60f-3a8df782f4a7%2Fimage.png)

- 서버에 있는 데이터를 스크립트에 의해 가공처리하여 만들어진 html문서를 전달하는 웹 페이지
...
- 웹 서버 이외에 추가적인 웹 어플리케이션 서버가 필요하므로 추가 비용